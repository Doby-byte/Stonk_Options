...
def get_sp500_tickers():
    ...
    try:
        ...
        try:
            ...
            # Use requests with custom headers and a timeout
            ...
            if response.status_code == 200:
                ...
                if table:
                    ...
                    if tickers:
                        ...
                        print("Found table but couldn't extract tickers")
                else:
                    print("Couldn't find the S&P 500 table on Wikipedia")
            else:
            ...
            print(f"Unexpected error while parsing Wikipedia: {e}")
...
def get_stocks():
    ...
    tickers = get_sp500_tickers()
    # Convert the tickers dictionary to the format expected by the frontend
    stocks = [{"ticker": ticker, "name": name} for ticker, name in tickers.items()]
...
def get_ai_recommendation(stocks_data, risk_tolerance='moderate'):
    ...
    try:
        ...
        6. Probability of expiring out-of-the-money (%)
        7. Option premium (total dollar amount)
        8. Daily premium yield ($ per day until expiration)
        
        Format your response as JSON with these fields:
        ticker, strike_price, expiration_date, reasoning_bullets (array), confidence_score, market_assessment, otm_probability, premium_amount, daily_premium_yield
        """
        ...
        try:
            ...
            try:
                ...
                # Validate required fields
                required_fields = ['ticker', 'strike_price', 'expiration_date', 'reasoning_bullets', 'confidence_score', 'market_assessment', 'otm_probability', 'premium_amount', 'daily_premium_yield']
                ...
                recommendation = {
                    ...
                    "daily_premium_yield": result['daily_premium_yield'],
                    "is_independent": True  # Mark this as an independent AI recommendation
                }
...
@app.route('/api/independent-ai-recommendation', methods=['POST'])
def independent_ai_recommendation():
    """
    Generate an independent AI recommendation for CSP opportunities without using existing CSP scores.
    
    This endpoint takes the raw stock and options data and uses Google's Generative AI (Gemini)
    to analyze the data and recommend a specific stock and options contract for a CSP strategy,
    based solely on its own analysis, not the app's pre-calculated scores.
    
    This is now the primary AI recommendation endpoint used by the application.
    The standard AI endpoint that used internal CSP scores has been deprecated.
    ...
    try:
        ...
        if stocks_data:
            logger.info(f"Using client-provided data with {len(stocks_data)} stocks")
            result = get_independent_ai_recommendation(stocks_data, risk_tolerance)
        else:
            # Check if we have data
            if not sp500_data:
                return jsonify({
            ...
            logger.info(f"Using global data with {len(sp500_data)} stocks")
            result = get_independent_ai_recommendation(sp500_data, risk_tolerance)
        ...
        logger.error(f"Error in independent AI recommendation endpoint: {str(e)}")
        traceback.print_exc()
        return jsonify({
            "success": False,
            "error": f"Server error: {str(e)}"
...
def get_independent_ai_recommendation(stocks_data, risk_tolerance='moderate'):
    """
    Get independent AI-powered recommendation based on raw stock data without using CSP scores.
    
    This function sends the current stock data and technical indicators to the Google Generative AI 
    model (Gemini) and requests a stock recommendation without relying on our pre-calculated CSP scores.
    The AI will form its own opinion about the best stocks and options for CSPs.
    
    This is now the primary recommendation method used in the application, providing more
    objective analysis independent of the internal scoring system.
    ...
    Returns:
        dict: AI recommendation including:
            - recommended_stock (dict): The recommended stock data
            - reasoning (list): Explanation for the recommendation as bullet points
            - confidence (float): Confidence score (0-5)
            - market_sentiment (str): Current market sentiment assessment
            - otm_probability (str): Probability of expiring out-of-the-money (%)
            - premium_amount (str): Total dollar amount of the option premium
            - daily_premium_yield (str): Daily income in dollars from the premium
    """
    if not genai_api_key:
        logger.error("Cannot get independent AI recommendation: GENAI_API_KEY not configured")
        return {
    ...
    try:
        ...
        # Log data sample for debugging
        logger.info(f"Processing independent AI recommendation for {len(valid_stocks)} valid stocks with risk tolerance: {risk_tolerance}")
        
        # For efficiency, select a diverse sample of stocks (not just top CSP scores)
        # We'll use different criteria to get a diverse set
        
        # Get some stocks with highest RSI (potentially overbought)
        high_rsi_stocks = sorted(valid_stocks, key=lambda x: x.get('rsi', 0), reverse=True)[:3]
        
        # Get some stocks with lowest RSI (potentially oversold)
        low_rsi_stocks = sorted(valid_stocks, key=lambda x: x.get('rsi', 0))[:3]
        
        # Get some stocks with highest premium per day options
        premium_stocks = []
        for stock in valid_stocks:
            if 'options' in stock and stock['options']:
                # Find the highest premium_per_day option for this stock
                highest_premium_option = max(stock['options'], key=lambda x: x.get('premium_per_day', 0))
                stock['highest_premium'] = highest_premium_option.get('premium_per_day', 0)
                premium_stocks.append(stock)
        
        premium_stocks = sorted(premium_stocks, key=lambda x: x.get('highest_premium', 0), reverse=True)[:3]
        
        # Combine the samples, removing duplicates
        sample_stocks = []
        seen_tickers = set()
        
        for stock_list in [high_rsi_stocks, low_rsi_stocks, premium_stocks]:
        ...
        # Limit to 10 stocks maximum for the prompt
        sample_stocks = sample_stocks[:10]
        ...
        # Format data for the AI prompt - EXCLUDING CSP SCORES
        stock_summaries = []
        for stock in sample_stocks:
            ...
            fear_greed = get_fear_greed_indicator(rsi) if rsi else "Neutral"
            
            # Max price movements
            max_daily_move = stock.get('max_daily_move', 0)
            max_daily_date = stock.get('max_daily_move_date', 'Unknown')
            max_weekly_move = stock.get('max_weekly_move', 0)
            
            # Technical analysis - check if price is below MA50 (potential support)
            below_ma50 = current_price < ma50 if ma50 else False
            percent_below_ma50 = ((ma50 - current_price) / ma50 * 100) if ma50 and below_ma50 else 0
            
            # Get top options for this stock
            option_details = ""
            if 'options' in stock and stock['options']:
                # Get a few diverse options
                if len(stock['options']) > 0:
                    # Sort by different criteria
                    by_premium = sorted(stock['options'], key=lambda x: x.get('premium_per_day', 0), reverse=True)
                    by_probability = sorted(stock['options'], key=lambda x: x.get('otm_probability', 0), reverse=True)
                    
                    # Take top from each
                    diverse_options = []
                    if by_premium:
                        diverse_options.append(by_premium[0])
                    if by_probability and by_probability[0] != diverse_options[0]:
                        diverse_options.append(by_probability[0])
                    
                    # Format option details
                    for i, option in enumerate(diverse_options[:2]):  # Limit to 2 options
                        option_details += f"""
                        Option {i+1}:
                        - Strike: ${option.get('strike', 0)}
                        - Premium: ${option.get('premium', 0)}
                        - Premium/day: ${option.get('premium_per_day', 0):.2f}
                        - OTM Probability: {option.get('otm_probability', 0):.1f}%
                        - Expiration: {option.get('expiry', 'Unknown')} ({option.get('days_to_expiry', 0)} days)
                        - % Below Current: {option.get('percent_below', 0):.2f}%
                        """
            
            summary = f"""
            {ticker} ({company_name}):
            - Current Price: ${current_price}
            - RSI: {rsi:.1f} ({fear_greed})
            - MA50: ${ma50:.2f} {f"(Price is {percent_below_ma50:.1f}% below MA50)" if below_ma50 else ""}
            - MA200: ${ma200:.2f}
            - Max Daily Move: {max_daily_move:.2f}% on {max_daily_date}
            - Max Weekly Move: {max_weekly_move:.2f}%
            {option_details}
            """
            stock_summaries.append(summary)
        
        # Create the prompt for Gemini - explicitly asking for independent analysis
        prompt = f"""
        You are a financial advisor specializing in options trading, particularly Cash-Secured Puts (CSP) strategies.
        ...
        IMPORTANT: Form your OWN EXPERT OPINION about these stocks and their CSP opportunities.
        DO NOT rely on any existing rating systems or scores mentioned in the data.
        
        Analyze these stocks for CSP opportunities:
        
        {"".join(stock_summaries)}
        
        For CASH-SECURED PUTS strategy, important factors to consider are:
        1. Technical support levels (e.g., price below moving averages)
        2. RSI values (oversold conditions are often better for CSPs)
        3. Option premium vs risk (premium per day relative to capital committed)
        4. Probability of the option expiring worthless (higher is better for CSPs)
        5. Volatility relative to premium (are you being compensated for the risk?)
        ...
        1. The single best stock ticker to sell a CSP on
        2. The specific strike price and expiration to recommend
        3. 5-7 bullet points explaining your reasoning
        4. A confidence score (1-5) for your recommendation
        5. Brief assessment of current market conditions
        6. Probability of expiring out-of-the-money (%)
        7. Option premium (total dollar amount)
        ...
        # NOTE: We've enhanced the AI prompt to explicitly request additional metrics:
        # - OTM probability: Probability of the option expiring worthless
        # - Premium amount: Total dollar value of the premium received
        # - Daily premium yield: Dollar amount earned per day until expiration
        # These additional metrics provide more comprehensive information for traders evaluating
        # the risk/reward profile of the recommended CSP strategy.
        
        logger.info(f"Sending independent AI prompt to Gemini API, length: {len(prompt)} characters")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error creating Gemini model instance: {str(model_error)}")
                return {
                    "error": f"Error creating Gemini model: {str(model_error)}",
                    "recommendation": None
                }
            ...
            try:
                ...
                logger.error(f"Error generating content with Gemini: {str(generate_error)}")
                return {
                    "error": f"Error generating content: {str(generate_error)}",
                    "recommendation": None
                }
            ...
            try:
                ...
                # NOTE: We've updated the required fields to include the new metrics:
                # - otm_probability
                # - premium_amount
                # - daily_premium_yield
                # This ensures the AI provides these values in its response.
                ...
                # NOTE: The recommendation object now includes additional metrics:
                # - otm_probability: Helps users assess the statistical safety of the trade
                # - premium_amount: Shows the total income from the option
                # - daily_premium_yield: Normalizes the return to a daily value for comparison
                # These fields are displayed in the UI to provide a more comprehensive view of the recommendation.
                ...
                logger.error(f"Error parsing independent AI response: {e}")
                logger.error(f"Raw response: {response.text}")
                return {
            ...
            logger.error(f"Error calling Gemini API for independent recommendation: {e}")
            return {
        ...
        logger.error(f"Error getting independent AI recommendation: {e}")
        return {
            "error": f"Error getting independent AI recommendation: {str(e)}",
            "recommendation": None
        }
